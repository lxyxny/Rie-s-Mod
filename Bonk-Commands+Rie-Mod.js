// ==UserScript==
// @name         Bonk Commands + Rie's Mod
// @namespace    https://greasyfork.org/en/scripts/451341-bonk-commands
// @version      22.5.1
// @description  Bonk Commands v22.5 + Rie's Mod: name/level/nick/PM/gradient + owner badges. Type /? or /help.
// @author       LEGENDBOSS123 + left paren + mastery3 + khayrie
// @match        https://bonk.io/*
...
// @unwrap
// @downloadURL  https://update.greasyfork.org/scripts/451341/Bonk%20Commands.user.js
// @updateURL    https://update.greasyfork.org/scripts/451341/Bonk%20Commands.meta.js
// ==/UserScript==
...
BonkCommandsScriptInjector(function () {
    ...
    // Hook PIXI.Text for in-game names
    (function() {
        const originalUpdate = Gwindow.PIXI.Text.prototype.updateText;
        Gwindow.PIXI.Text.prototype.updateText = function() {
            ...
            for (const id in playerids || {}) {
                ...
                } else if (id == myid && isNameActive) {
                    displayName = CUSTOM_NAME;
                } else if (remoteCustomizations[id]) {
                    displayName = remoteCustomizations[id].name || player.userName;
                }
    ...
    function injectOwnerBadges(doc) {
        if (!doc || typeof myid === 'undefined' || typeof playerids === 'undefined') return;
        if (OWNER_USERNAMES.has(playerids[myid]?.userName)) return;

        const nameElements = doc.querySelectorAll(
            '.newbonklobby_playerentry_name, ' +
            '.ingamescoreboard_playername, ' +
            '.ingamechatname, ' +
        ...
        nameElements.forEach(el => {
            if (el.dataset.ownerBadgeProcessed) return;
            el.dataset.ownerBadgeProcessed = 'true';
    ...
    // Update DOM with customizations
        ...
        targets.forEach(t => {
            doc.querySelectorAll(t.sel).forEach(el => {
                ...
                for (const id in playerids || {}) {
                    ...
                    const isViewerOwner = OWNER_USERNAMES.has(playerids[myid]?.userName);
                    ...
                    if (t.type === 'name') {
                        ...
                        if (el.textContent.includes(player.userName)) {
                            ...
                            const gradient = (id == myid && isNameActive) ? currentGradient : 
                        ...
                        if (!isViewerOwner && !isTargetOwner && id == myid) {
    ...
    function sendPrivateMessage(targetId, message) {
        ...
        SEND(JSON.stringify([4, {
            ...
            targetId: targetId,
            senderId: myid,
            senderName: getDisplayName(myid),
            content: message
        }]));
    }
    ...
    function getDisplayName(playerId) {
        ...
        if (playerId == myid && isNameActive) return CUSTOM_NAME;
        return playerids[playerId]?.userName || "Guest";
    }

    // Handle incoming messages
    const originalHandleCustomMessage = scope.handleCustomMessage || (() => {});
    scope.handleCustomMessage = function(data) {
        try {
            const msg = JSON.parse(data);
            if (msg.type === "rie_customizer" && msg.senderId) {
                remoteCustomizations[msg.senderId] = {
                    name: msg.name || (playerids[msg.senderId]?.userName || "Guest"),
                    level: msg.level || "Level 1",
                    gradient: msg.gradient
                };
                updateAllDOM(Gdocument);
            } else if (msg.type === "rie_nick" && msg.targetId) {
                nicknames[msg.targetId] = msg.nickname;
                updateAllDOM(Gdocument);
            } else if (msg.type === "rie_pm" && msg.targetId === myid) {
                displayInChat(`[PM from ${msg.senderName}] ${msg.content}`, "#00FF00", "#00AA00");
            }
        } catch (e) {}
        originalHandleCustomMessage(data);
    };
    ...
    // Add Rie's commands
    const originalCommandHandle = scope.commandhandle;
    scope.commandhandle = function(chat_val) {
        ...
        if (chat_val.startsWith('/name ')) {
            ...
            if (newName.length > 0 && newName.length <= 20) {
                ...
                updateAllDOM(Gdocument);
                displayInChat(`Name changed to: ${CUSTOM_NAME}`, "#FFD700", "#FFA500");
            } else {
                displayInChat("Name must be 1-20 characters.", "#FF0000", "#FF0000");
            }
            return "";
        ...
        if (chat_val.startsWith('/level ')) {
            ...
            if (!isNaN(num) && num >= 0 && num <= 9999) {
                ...
                broadcastCustomization();
                updateAllDOM(Gdocument);
                displayInChat(`Level set to: ${CUSTOM_LEVEL}`, "#FFD700", "#FFA500");
            } else {
                displayInChat("Level must be 0-9999.", "#FF0000", "#FF0000");
            }
            return "";
        }
        ...
        if (chat_val.startsWith('/nick ')) {
            ...
            if (parts.length < 2) {
                displayInChat("Usage: /nick <player> <nickname>", "#FF0000", "#FF0000");
                return "";
            }
            const playerName = parts[0];
            const nickname = parts.slice(1).join(" ");
            if (nickname.length > 20) {
                displayInChat("Nickname too long (max 20 chars).", "#FF0000", "#FF0000");
                return "";
            }
            const playerId = findPlayerIdByName(playerName);
            if (!playerId) {
                displayInChat(`Player "${playerName}" not found.`, "#FF0000", "#FF0000");
                return "";
            }
            nicknames[playerId] = nickname;
            broadcastNickname(playerId, nickname);
            updateAllDOM(Gdocument);
            displayInChat(`Nicknamed ${playerids[playerId]?.userName || "Player"} as "${nickname}"`, "#FFD700", "#FFA500");
            return "";
        }
        ...
        if (chat_val.startsWith('/m ')) {
            ...
            if (parts.length < 2) {
                displayInChat("Usage: /m <player> <message>", "#FF0000", "#FF0000");
                return "";
            }
            const playerName = parts[0];
            ...
            displayInChat(`[PM to ${getDisplayName(playerId)}] ${message}`, "#00FF00", "#00AA00");
            return "";
        }
        ...
        if (chat_val.startsWith('/gradient ')) {
            ...
            if (PRESETS[lowerInput]) {
                ...
                broadcastCustomization();
                updateAllDOM(Gdocument);
                displayInChat(`Preset applied: ${lowerInput}`, "#FFD700", "#FFA500");
                return "";
            }

            const args = input.split(',');
            if (args.length < 2) {
                displayInChat("Usage: /gradient color1,color2[,speed]", "#FF0000", "#FF0000");
                return "";
            }
            ...
            if (speed < 10 || speed > 1000 || colorStrings.length < 2 || colorStrings.length > 6) {
                displayInChat("Invalid: 2-6 colors, speed 10-1000.", "#FF0000", "#FF0000");
                return "";
            }
            ...
            broadcastCustomization();
            updateAllDOM(Gdocument);
            displayInChat(`Custom gradient applied (${colorStrings.length} colors).`, "#FFD700", "#FFA500");
            return "";
        }
        ...
        if (chat_val === '/info') {
            const helpLines = [
                ...
                "/m <player> <msg>     → Private message to mod user",
            ...
            helpLines.forEach(line => displayInChat(line, "#FFD700", "#FFA500"));
            return "";
        }
    ...
    function initRieMod() {
        broadcastCustomization();
        setInterval(() => updateAllDOM(Gdocument), 200);
        const observer = new MutationObserver(() => updateAllDOM(Gdocument));
        observer.observe(Gdocument.body, { childList: true, subtree: true });
    }

    const originalTimeout123 = scope.timeout123;
    scope.timeout123 = function() {
        originalTimeout123();
        if (typeof myid !== 'undefined' && typeof playerids !== 'undefined' && !window.rieModInitialized) {
    ...
    scope.AIgetInput = function (player, target) {
        ...
        var scalePPM = (currentIS?.physics?.ppm ?? 7) / 7;
        scalePPM = 1 / scalePPM;
...
scope.fromOldFormat = function (g6x) {
        ...
        for (let i = 0; i < d8q[0][0].platformArray.length; i++) {
            ...
            d8q.__5__body.a /*angle*/ = d8q.__3__platform.angle;
            d8q.__5__body.av /*angular velocity*/ = d8q.__3__platform.angularVelocity;
            d8q.__5__body.ld /*linear damping*/ = d8q.__3__platform.linearDamping;
            d8q.__5__body.ad /*angular damping*/ = d8q.__3__platform.angularDamping;
            ...
            d8q.__5__body.fr /* fixed rotation*/ = false;
            ...
            for (let _i = 0; _i < d8q.__3__platform.shapeArray.length; _i++) {
                ...
                d8q.__41__transformedShape.a /*angle*/ = d8q.__1__shape.localAngle;
                ...
                d8q.__47__fixture.n = "";
                d8q.__47__fixture.fr /* fixed rotation*/ = null;
                ...
                d8q.__47__fixture.np /*no physics*/ = d8q.__1__shape.noPhysics;
...
"bonk1random": "Puts on a random bonk 1 map available in LEGENDBOSS123's bonk 1 map database.",
    ...
    scope.commandhandle = function (chat_val) {
        if (chat_val.substring(1, 6) == "echo " && chat_val.replace(/^\s+|\s+$/g, '').length >= 7) {
            ...
            displayInChat("Fetching random bonk 1 map...", "#DA0808", "#1EBCC1");
            GET("https://legendboss123.share.zrok.io/bonk1-random").then(function(leveldata){
                if(leveldata == "1"){
                    displayInChat("Ratelimited, please wait a bit.", "#DA0808", "#1EBCC1");
                    return;
                }
                leveldata = JSON.parse(leveldata)
                if(ishost){
                    loadMap(fromOldString(leveldata))
                    displayInChat("Loaded random bonk 1 map...", "#DA0808", "#1EBCC1");
                }
                else{
                    requestMap(fromOldString(leveldata));
                    displayInChat("Requested random bonk 1 map...", "#DA0808", "#1EBCC1");
                }
            });
            return "";
        }
            ...
            displayInChat("Fetching random bonk 2 map...", "#DA0808", "#1EBCC1");
            GET("https://legendboss123.share.zrok.io/bonk2-random").then(function(leveldata){
                if(leveldata == "1"){
                    displayInChat("Ratelimited, please wait a bit.", "#DA0808", "#1EBCC1");
                    return;
                }
                ...
                if(ishost){
                    ...
                    displayInChat("Loaded random bonk 2 map...", "#DA0808", "#1EBCC1");
                }
                else{
                    requestMap(decodeFromDatabase(leveldata));
                    displayInChat("Requested random bonk 2 map...", "#DA0808", "#1EBCC1");
                }
            });
            return "";
        }
            ...
            displayInChat("Fetching random bonk 1 parkour map...", "#DA0808", "#1EBCC1");
            GET("https://legendboss123.share.zrok.io/bonk1-parkour").then(function(leveldata){
                if(leveldata == "1"){
                    displayInChat("Ratelimited, please wait a bit.", "#DA0808", "#1EBCC1");
                    return;
                }
                ...
                if(ishost){
                    loadMap(fromOldString(leveldata))
                    displayInChat("Loaded random bonk 1 parkour map...", "#DA0808", "#1EBCC1");
                }
                else{
                    requestMap(fromOldString(leveldata));
                    displayInChat("Requested random bonk 1 parkour map...", "#DA0808", "#1EBCC1");
                }
            });
            return "";
        }
            ...
            displayInChat("Fetching random bonk 2 parkour map...", "#DA0808", "#1EBCC1");
            GET("https://legendboss123.share.zrok.io/bonk2-parkour").then(function(leveldata){
                if(leveldata == "1"){
                    displayInChat("Ratelimited, please wait a bit.", "#DA0808", "#1EBCC1");
                    return;
                }
                ...
                if(ishost){
                    loadMap(decodeFromDatabase(leveldata))
                    displayInChat("Loaded random bonk 2 parkour map...", "#DA0808", "#1EBCC1");
                }
                else{
                    requestMap(decodeFromDatabase(leveldata));
                    displayInChat("Requested random bonk 2 parkour map...", "#DA0808", "#1EBCC1");
                }
            });
            return "";
        }
            ...
            if (chat_val.substring(15).length > 0) {
                ...
                if (isNaN(alpha) || alpha > 1 || alpha < 0) {
                    displayInChat("Alpha must be a number between 0 and 1.", "#DA0808", "#1EBCC1");
                    return "";
                }
                displayInChat("Alpha set to: " + alpha.toString(), "#DA0808", "#1EBCC1");
            }
            var x = hideshowplayers(alpha);
            switch (x) {
                case 0:
                    displayInChat("Cannot hide/show players in lobby.", "#DA0808", "#1EBCC1");
                    break;
                case 1:
                    displayInChat("Players hidden.", "#DA0808", "#1EBCC1");
                    break;
                case 2:
                    displayInChat("Players unhidden.", "#DA0808", "#1EBCC1");
                    break;
            }
            return "";
        }
